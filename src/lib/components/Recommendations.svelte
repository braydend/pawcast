<script lang="ts">
	...
	import { getTemperatureRecommendations } from "$lib/domain/recommendation";
    ...
    const recommendationCollections = forecasts.map<Recommendation>((forecast) => {
...
<section class="col-start-1 col-span-2 gap-4 flex flex-col">
    ...
    {#each recommendationCollections as recommendation}
        <Card.Root class={borderColours[gradeToColour(recommendation.grade)]}>
            ...
            <Card.Content>
                {#each getTemperatureRecommendations(recommendation.grade) as message}
                    <p>{message}</p>
                {/each}
            </Card.Content>
        </Card.Root>
